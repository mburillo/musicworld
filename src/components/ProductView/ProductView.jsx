...
const carouselImageStyle = {
    ...
    const fetchReviews = async () => {
        try {
            const response = await axios.get(`http://localhost:8080/api/reviews/${id}?page=${reviewPage}&size=20&sort=${sortOption}`);
    ...
    const handleAddReview = async () => {
        try {
            ...
            await axios.post('http://localhost:8080/api/reviews', { content: newReview, rating: rating, productId:id }, {
                headers: {
    ...
    <select onChange={handleSortChange} value={sortOption}>
        ...
        <option value="oldest">Oldest</option>
            ...
            <button className="btn btn-outline-secondary mt-3" onClick={handleLoadMoreReviews}>Load more reviews</button>
        </div>
            </div>
        </div>
</>
